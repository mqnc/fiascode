...
namespace
{
	...
	public:
		...
		Iterator& front() const {return *mBegin;}
		void popFront() {++mBegin;}
		void popEnd() {--mEnd;}
		friend Iterator const& begin(iterator_range const& rng) {return rng.mBegin;}
		friend Iterator const& end(iterator_range const& rng) {return rng.mEnd;}
 
	private:
		Iterator mBegin; // first included element
		Iterator mEnd; // first no longer included element
	};
}
